// <![CDATA[
var speed=55; // lower number for faster
var untidy=1; // how often do you want the flakes tidied up (high number is less often)
var sizes=56; // maximum size of flakes in pixels
...
var snowflakes=new Array(37, 51, 68, 75, 81, 107, 112, 114, 118, 120, 121);
var ie_version=(navigator.appVersion.indexOf("MSIE")!=-1)?parseFloat(navigator.appVersion.split("MSIE")[1]):false;
var plow=document.createElement("img");
plow.src='http://thecroc.com/christmas/img/plough.png';
...
function freeze_ice(whyp) {
  ...
  //---------- MODified by  MMB ---------
  if(snowflake_db_active){
	  
	  var ramdompos = Math.floor(Math.random()*snowflake_data_len);
	  var allow_message = snowflake_items[Math.floor(Math.random()*snowflake_items.length)];
	  
	 if(allow_message == 1){
		 //console.log()
		if (typeof ramdompos !== typeof undefined){
			
			f.className = snowflake_data[ramdompos].id;
			fuk=document.createElement("div");
			fuk.className = 'tooltip';
			xx=document.createTextNode(snowflake_data[ramdompos].to);
			fuk.appendChild(xx);
			f.appendChild(fuk);
		}
	 }
  ...
  for (i=0; i<starty; i++) {
    if (flaky[i] && le[i]!='tidying') {
		...
		//MOD MMB
			...
			//---------- MODified by  MMB ---------
			var x = flaky[i].getElementsByTagName("*");
			if (typeof x[0] !== typeof undefined) x[0].style.display = 'none';
			//---------- END MODified by  MMB ---------
		};
	}
